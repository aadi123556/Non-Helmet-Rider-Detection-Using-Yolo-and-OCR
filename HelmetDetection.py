...
def drawPred(classId, conf, left, top, right, bottom,frame,option):
    ...
    if label_name == 'Helmet' and conf > 0.50:
        ...
        if option == 0 and conf < 0.90:
            ...
            img = cv.imread('labels/'+str(predict)+".png")
...
font = ('times', 16, 'bold')
title = Label(main, text='Detection of Non-Helmet Riders and Extraction of License Plate Number using Yolo v2 and OCR Method', justify=LEFT)
...
videoimage = Button(main, text="Helmet Detection From Video", command=videoHelmetDetect)
videoimage.place(x=200,y=250)
videoimage.config(font=font1) 
